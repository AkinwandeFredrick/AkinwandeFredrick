...
def fetch_latest_repos(n: int = 5) -> list[dict]:
    ...
    repos = gh_get(
        f"https://api.github.com/users/AkinwandeFredrick/repos",
...
def fetch_user_stats() -> dict:
    ...
    u = gh_get(f"https://api.github.com/users/AkinwandeFredrick")
    return {
...
def fetch_recent_activity(n: int = 5) -> list[str]:
    ...
    events = gh_get(
        f"https://api.github.com/users/AkinwandeFredrick/events/public",
        params={"per_page": 30},
    )
    lines = []
    type_map = {
...
def build_last_updated_section() -> str:
    ...
    return f"![Last Updated](https://img.shields.io/badge/Last%20Updated-{now.replace(' ', '%20').replace(':', '%3A')}-brightgreen?style=flat-square)"
